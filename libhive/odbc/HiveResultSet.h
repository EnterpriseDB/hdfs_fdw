...
class HiveResultSet {
  public:
    ...
    virtual HiveReturn getFieldDataLen(size_t column_idx, size_t* col_len, char* err_buf, size_t err_buf_len) = 0;
    virtual HiveReturn getFieldAsCString(size_t column_idx, char* buffer, size_t buffer_len, size_t* data_byte_size, int* is_null_value, char* err_buf, size_t err_buf_len) = 0;
...
class HiveQueryResultSet: public HiveResultSet {
  public:
    ...
    virtual HiveReturn initialize(HiveConnection* connection, string query_str, char* err_buf, size_t err_buf_len);
    virtual HiveReturn fetchNext(char* err_buf, size_t err_buf_len) = 0;
...
class HiveQueryResultSetHS1: public HiveQueryResultSet {
  public:
    HiveQueryResultSetHS1(int max_buf_rows);
    virtual ~HiveQueryResultSetHS1();
    virtual HiveReturn initialize(HiveConnection* connection, string query_str, char* err_buf, size_t err_buf_len);
    virtual HiveReturn fetchResults(TOperationHandle &opHandle);
    virtual HiveReturn getValue(string& val, TColumnValue& col_value);
    virtual HiveReturn fetchNext(char* err_buf, size_t err_buf_len);
    virtual size_t getColumnCount();
    virtual HiveReturn getFieldDataLen(size_t column_idx, size_t* col_len, char* err_buf, size_t err_buf_len);
    virtual HiveReturn getFieldAsCString(size_t column_idx, char* buffer, size_t buffer_len, size_t* data_byte_size, int* is_null_value, char* err_buf,size_t err_buf_len);
    virtual HiveReturn hasResults(int* results, char* err_buf, size_t err_buf_len);
    virtual HiveReturn getColumnCount(size_t* col_count, char* err_buf, size_t err_buf_len);
    virtual HiveReturn createColumnDesc(size_t column_idx, HiveColumnDesc** column_desc_ptr, char* err_buf, size_t err_buf_len);
    virtual HiveRowSet& getRowSet();
    virtual HiveReturn initializeSchema(char* err_buf, size_t err_buf_len);
    virtual HiveReturn fetchNewResults(char* err_buf, size_t err_buf_len);
  private: 
    
};

class HiveQueryResultSetHS2: public HiveQueryResultSet {
  public:
    HiveQueryResultSetHS2(int max_buf_rows);
    virtual ~HiveQueryResultSetHS2();
    virtual HiveReturn initialize(HiveConnection* connection, string query_str, char* err_buf, size_t err_buf_len);
    virtual HiveReturn fetchResults(TOperationHandle &opHandle);
    virtual HiveReturn getValue(string& val, TColumnValue& col_value);
    virtual HiveReturn fetchNext(char* err_buf, size_t err_buf_len);
...
class HiveTablesResultSet: public HiveResultSet {
  public:
    ...
    virtual HiveReturn getFieldDataLen(size_t column_idx, size_t* col_len, char* err_buf, size_t err_buf_len) { return HIVE_SUCCESS; }
    virtual HiveReturn getFieldAsCString(size_t column_idx, char* buffer, size_t buffer_len, size_t* data_byte_size, int* is_null_value, char* err_buf, size_t err_buf_len) { return HIVE_SUCCESS; }
