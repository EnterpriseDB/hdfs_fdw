...
hdfs_fdw_xact_callback(XACT_CB_SIGNATURE)
{
	...
	if (nestingLevel > 0)
	{
		if (nestingLevel > 1)
			ereport(DEBUG1,
					(errmsg("hdfs_fdw: %d connections closed", nestingLevel)));
		else
			ereport(DEBUG1,
					(errmsg("hdfs_fdw: %d connection closed", nestingLevel)));
	}
}
					  ...
					  Oid foreigntableid)
{
	...
	if (IS_DEBUG)
		ereport(LOG,
				(errmsg("hdfs_fdw: hdfsGetForeignRelSize starts [%d]", con_index)));
	...
	if (IS_DEBUG)
		ereport(LOG,
				(errmsg("hdfs_fdw: hdfsGetForeignRelSize ends [%f]",
						baserel->rows)));
}
...
hdfsGetForeignPaths(PlannerInfo *root, RelOptInfo *baserel, Oid foreigntableid)
{
	...
	if (IS_DEBUG)
		ereport(LOG,
				(errmsg("hdfs_fdw:hdfsGetForeignPaths starts ")));
	...
	if (IS_DEBUG)
		ereport(LOG,
				(errmsg("hdfs_fdw:hdfsGetForeignPaths ends")));
}
...
#endif
{
	...
	if (IS_DEBUG)
		ereport(LOG,
				(errmsg("hdfs_fdw: hdfsGetForeignPlan starts")));
	...
	if (IS_DEBUG)
		ereport(LOG,
				(errmsg("hdfs_fdw:hdfsGetForeignPlan ends")));
...
hdfsBeginForeignScan(ForeignScanState *node, int eflags)
{
	...
	if (IS_DEBUG)
		ereport(LOG,
				(errmsg("hdfs_fdw:hdfsBeginForeignScan starts")));
	...
	if (IS_DEBUG)
		ereport(LOG,
				(errmsg("hdfs_fdw:hdfsBeginForeignScann param count [%d]",
						numParams)));
	...
	if (IS_DEBUG)
		ereport(LOG,
				(errmsg("hdfs_fdw:hdfsBeginForeignScann ends")));
}
...
hdfsIterateForeignScan(ForeignScanState *node)
{
	...
	if (IS_DEBUG)
		ereport(LOG,
				(errmsg("hdfs_fdw:hdfsIterateForeignScan starts")));
	...
	if (IS_DEBUG)
		ereport(LOG,
				(errmsg("hdfs_fdw:hdfsIterateForeignScan ends")));
	return slot;
}
